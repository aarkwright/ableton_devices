...
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\control_surface.py
    ...
    def _install_forwarding(self, midi_map_handle, control, forwarding_type=ScriptForwarding.exclusive):
        ...
        if success:
            ...
            for key in forwarding_keys:
                ...
                if not key not in registry.keys():
                    raise AssertionError(b'Registry key %s registered twice. Check Midi messages!' % str(key))
