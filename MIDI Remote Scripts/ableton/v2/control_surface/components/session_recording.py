...
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\session_recording.py
...
class SessionRecordingComponent(Component):
    ...
    """
    _automation_toggle = ToggleButtonControl(toggled_color=b'Automation.On', untoggled_color=b'Automation.Off')
    _re_enable_automation = ButtonControl(color=b'Automation.On')
    _delete_automation = ButtonControl(color=b'Automation.Off')
    ...
    def _update_record_button(self):
        if self.is_enabled():
            ...
            if status == Live.Song.SessionRecordStatus.transition:
                self._record_button.color = b'Recording.Transition'
            elif status == Live.Song.SessionRecordStatus.on or song.session_record:
                self._record_button.color = b'Recording.On'
            else:
                self._record_button.color = b'Recording.Off'
    ...
    def _update_delete_automation_button_color(self):
        if self._on_playing_clip_has_envelopes_changed.subject:
            self._delete_automation.color = b'Automation.On' if self._on_playing_clip_has_envelopes_changed.subject.has_envelopes else b'Automation.Off'
