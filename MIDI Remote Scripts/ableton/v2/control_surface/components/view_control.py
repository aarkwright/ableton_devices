...
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\view_control.py
...
class ViewControlComponent(Component):
    ...
    def focus_view(self, view):
        if not view in VIEWS:
            ...
            app_view = self.application.view
            if not ((view == b'Detail/DeviceChain' or b'Detail/Clip') and app_view.is_view_visible(b'Detail')):
                app_view.show_view(b'Detail')
        if not app_view.is_view_visible(view):
