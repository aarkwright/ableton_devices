...
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\base\util.py
...
def monkeypatch(target, name=None, override=False, doc=None):
    ...
    def patcher(func):
        ...
        if not override and hasattr(target, patchname):
            raise TypeError(b'Class %s already has method %s' % (target.__name__,
             patchname))
