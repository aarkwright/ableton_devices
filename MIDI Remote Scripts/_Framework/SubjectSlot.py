...
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\SubjectSlot.py
...
class SubjectSlot(Disconnectable):
    ...
    def _check_subject_interface(self, subject):
        if not callable(getattr(subject, b'add_' + self._event + b'_listener', None)):
            raise SubjectSlotError(b'Subject %s missing "add" method for event: %s' % (
             subject, self._event))
        ...
        if not callable(getattr(subject, self._event + b'_has_listener', None)):
            raise SubjectSlotError(b'Subject %s missing "has" method for event: %s' % (
             subject, self._event))
        return
